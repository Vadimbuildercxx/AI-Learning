...
class KNearestNeighbor:
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            #######################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            diff_vector = np.subtract(self.X_train, X[i])
            square_elementwise = np.multiply(diff_vector, diff_vector)
            dists[i,:] = np.sqrt(np.matmul(square_elementwise, np.ones((square_elementwise.shape[1],))))
            # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ...
    def compute_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        # x2 = np.dot(self.X_train, self.X_train.T)
        # y2 = np.dot(X, X.T)
        x2 = np.sum(X**2, axis=1) # shape of (m)
        y2 = np.sum(self.X_train**2, axis=1) # shape of (n)
